...
export default function Nurse() {
    ...
    const [nurseData, setData] = useState([])
    useEffect(() => {
        AxiosInstance.get(`staffs/`)
            .then(res => {
                const filtered = res.data.filter(nurseData => nurseData.position === 'N')
                setData(filtered)
            })
    ...
    const handleSearchChange = (searchValue) => {
        ...
        if (!searchValue.trim()) {
            ...
            const filtered = nurseData.filter(item => item.name.toLowerCase().includes(searchValue.toLowerCase()));
    ...
    const handleSortChange = (type) => {
        ...
        const sorted = [...nurseData].sort((a, b) => {
            if (type === 'name') {
    ...
    const totalData = filteredData.length > 0 ? filteredData.length : sortedData.length > 0 ? sortedData.length : nurseData.length;

    // Tính toán vị trí của dữ liệu trên trang hiện tại
    const indexOfLastData = currentPage * dataPerPage;
    const indexOfFirstData = indexOfLastData - dataPerPage;
    const currentData = filteredData.length > 0 ? filteredData.slice(indexOfFirstData, indexOfLastData) : sortedData.length > 0 ? sortedData.slice(indexOfFirstData, indexOfLastData) : nurseData.slice(indexOfFirstData, indexOfLastData);
