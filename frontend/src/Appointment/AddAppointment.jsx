...
export default function AddApointment() {
    ...
    const [value, setValue] = useState({
        time: '',
        date: '',
        doctorname: nameFromEditStaff ? nameFromEditStaff : '',
        patientname: '',
        feestatus: false, // Thiết lập mặc định là false
        completed: false, // Thiết lập mặc định là false
    });
    ...
    const handleDoctorInputChange = (event) => {
        ...
        setDoctorSearchText(inputText);
    };

    const handlePatientInputChange = (event) => {
        const inputText = event.target.value;
        setPatientSearchText(inputText);
    };
    ...
    const filterPatients = () => {
        return patient.filter(pat =>
            pat.name.toLowerCase().includes(patientSearchText.toLowerCase())
        );
    };

    const handleSubmit = (event) => {
        event.preventDefault();
        AxiosInstance.post(`appointments/`, value)
            .then(res => {
                console.log(res);
            })
            .catch(err => console.log(err));
                        ...
                        <div className="mx-5" style={{ width: 600 }}>
                            <form onSubmit={handleSubmit}>
                                <div className="row">
                                    ...
                                    <div className="col-8">
                                        <label  style={{ fontWeight: 'bold' }}  htmlFor="doctorInput">Select Doctor:</label>
                                        <div className="d-flex align-items-center">
                                            <select className="form-control ms-2" value={value.doctorname} onChange={(e) => setValue({ ...value, doctorname: e.target.value })}>
                                        ...
                                        <label className="mt-3"  style={{ fontWeight: 'bold' }} >Select Patient:</label>
                                        <div className="d-flex align-items-center">
                                            <select className="form-control ms-2" value={value.patientname} onChange={(e) => setValue({ ...value, patientname: e.target.value })}>
                                                <option value="" disabled>Select a patient</option>
                                                {filterPatients().map((pat, index) => (
                                ...
                                <div className="row mt-3">
                                    <div className="col-4">
                                        ...
                                        <input className="form-control" type="date" value={value.date} onChange={(e) => setValue({ ...value, date: e.target.value })}></input>
                                    </div>
                                    <div className="col-4">
                                        <label style={{ fontWeight: 'bold' }}>Time:</label>
                                        <input className="form-control" type="time" value={value.time} onChange={(e) => setValue({ ...value, time: e.target.value })}></input>
                                    </div>
                                    <div className="col-4">
                                        <label style={{ fontWeight: 'bold' }}>{value.feestatus===false? <div style={{color:'red'}}>Unpaid</div> : <div style={{color:'green'}}>Paid</div> }</label>
                                        <div className="form-check form-switch ms-2">
                                            < input className="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault" checked={value.feestatus} onClick={e => setValue({ ...value, feestatus: !value.feestatus })} readOnly />
                                        </div>
                                    </div>
                                </div>
