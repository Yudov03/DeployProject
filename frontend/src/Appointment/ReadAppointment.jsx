...
export default function ReadAppointment() {
    ...
    // Tính toán thời gian bắt đầu của sự kiện so với hiện tại
    const calculateBeginTime = () => {
        const eventDate = moment(data.date, 'YYYY-MM-DD');
        const currentDate = moment();
        const dayDifference = eventDate.diff(currentDate, 'days');

        if (dayDifference > 0) {
            return `After ${dayDifference} day(s)`;
        } else if (dayDifference === 0) {
            return 'Today';
        } else {
            return `Event occurred ${Math.abs(dayDifference)} day(s) ago`;
        }
    };
    return (
        <>
                        ...
                        <div className="d-flex justify-content-center border border-1 border-primary w-100">
                            <div className="d-flex flex-column w-100 ms-2" >
                                <div className="col mt-2 ms-2">
                                    <div className="row">
                                        <div className="mt-1 fw-bold">Appointment between Dr. {data.doctorname} and {data.patientname}</div>
                                        
                                    </div>
                                    <div className="row mt-2">
                                        <div>Begin at {data.time} {data.date}</div>
                                    </div>
                                    <div className="d-flex justify-content-between mt-2">
                                        <div className="">{data.feestatus ? <div style={{ color: 'green' , fontWeight: 'bold'}}>Paid</div> : <div style={{ color: 'red', fontWeight: 'bold' }}>Unpaid</div>}</div>
                                        <div className="me-3">{data.completed===true? <div style={{color:'green', fontWeight: 'bold'}}>Completed</div> : <div style={{color:'red', fontWeight: 'bold'}}>Uncompleted</div>}</div>
                                    </div>
                                    {data.completed &&
                                    <div className="d-flex justify-content-between mt-2">
                                        <div className="fw-bold">Diagnosis: {data.diagnosis}</div>
                                        <div className="me-3 d-flex fw-bold"><div className="me-1 ">Rate:</div> {data.rate === "E"?"Excellent": <div>{data.rate==="G"?"Good": <div>{data.rate==="O"?"Ok": <div>{data.rate==="P"?"Poor":"Terrible"}</div> }</div> }</div> }</div>
                                    </div>}
                                </div>
                            </div>
                        </div>
